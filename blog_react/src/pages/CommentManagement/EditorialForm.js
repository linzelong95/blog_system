...
const {AdminCommentAPI:{INSERT},AdminArticleAPI}=UrlEnum;
...
class EditorialForm extends React.PureComponent {
  ...
  getSelectorConfig = ({ container,...otherProps}) => {
    ...
    return {
      pagination: {
        ...
        onChange: (i, s) => this.searchItem({ query: container.query, container, pageIndex: i, pageSize: s})
      },
  ...
  render() {
    ...
    const modalFormConfig = [
      { fieldId: 'aid', label:"文章",rules: [{ required: true, message: "文章是必须的" }], fieldType: 'select', fieldProps: { ...this.getSelectorConfig({container:articlecontainer}), style: { width: "86%" } },formItemLayout: { labelCol: { span: 6 } }},
      { fieldId: 'is_top', label:"是否置顶", rules: [{ required: true, message: "该项是必须的" }],fieldType: 'select', fieldProps: { options: [{value:1,label:"是"},{value:0,label:"否"}], style: { width: "86%" } },initialValue:1, formItemLayout: { labelCol: { span: 6 } }},
      { fieldId: 'content', label: "评论内容", rules: [{ required: true, message: "评论内容是必须的" }],fieldProps: { style: { width: "86%" }, autosize: true }, fieldType: 'textArea', formItemLayout: { labelCol: { span: 6 } } },
    ];
    return getModalForm({  editorialPanelVisible, toggleEdit: this.toggleEdit, form, modalFormConfig, INSERT, UPDATE:INSERT, width: 600 });
  }
}
