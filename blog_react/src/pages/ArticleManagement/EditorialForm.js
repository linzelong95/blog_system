...
const { AdminArticleAPI: { INSERT, UPDATE,CONTENT } ,AdminCateAPI} = UrlEnum;
...
class EditorialForm extends React.PureComponent {
  ...
  formatInitialFormData = (formItem, extraObj) => {
    const { category_id, category_name, content, sort,image_url } = formItem;
    console.log(`${imgPrefix}${image_url}`)
    ...
    form.validateFields((err, values) => {
      ...
      fileList.forEach(i => {
        if (i.url) {
          ...
          i.response.list.forEach(v => { image_urls.push(v.url) });
        }
      });
